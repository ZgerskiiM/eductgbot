...
def get_text_messages(bot, cur_user, message):
    ...
    # ======================================= реализация игры Камень-ножницы-бумага
        ...
        gameRSP.newGame()
    # =======================================
    elif ms_text == "Игра Путанница":
        word_game = getGame(chat_id)
        if word_game is None:
            word_game = Word_game(bot, chat_id)
            newGame(chat_id, word_game)
            return
...
class Word_game:
    ...
    name = "Путаница"
    description = "Правила:\n" \
                 "1.Дается перемешанное слово\n" \
                 "2.Нужно угадать какое это слово\n"\
                 "3.Для выхода напиши 'хватит'\n"


    list_words = ["карта", "луна", "ограбление", "привидение", "наушники", "газировка", "книга", "подарок", "тетрадь", "попугай", "сигарета", "люстра", "бутылка"]

    def __init__(self, bot, chat_id):
        self.winner = None
        self.chat_id = chat_id
        self.computerChoice = self.__class__.get_random_word()
        self.mixed_word = self.__class__.mix(self.computerChoice)
    ...
    def newGame(self):
        self.computerChoice = self.__class__.get_random_word()
        text = f"вы играете в игру \n {self.__class__.name}\n {self.__class__.description}\n Вот тебе задачка {self.mixed_word} "
        self.__class__.bot.send_message(self.chat_id, text=text)

    @classmethod
    def get_random_word(cls):
        import random
        return random.choice(cls.list_words)

    @classmethod
    def mix(self, word):
        return word[::-1]

    def playerChoice(self, playerChoice):
        winner = None
        if playerChoice != "Хватит":

            if playerChoice == self.computerChoice:
                winner = "Вы выиграли"
            else:
                winner = "Вы проиграли. Попробуй еще раз"
            self.__class__.bot.send_message(self.chat_id, text=winner)
            self.newGame()
